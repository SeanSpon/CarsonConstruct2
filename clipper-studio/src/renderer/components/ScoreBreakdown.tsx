        ...
        <div className="mt-3 space-y-3 animate-in slide-in-from-top-2 duration-200">
          ...
          <div>
            ...
            <div className="space-y-1.5">
              {/* Pattern Score */}
              {(clip.algorithmScore !== undefined || softScores?.payoff_score || softScores?.monologue_score) && (
                <ScoreBar
                  label="Pattern Detection"
                  score={clip.algorithmScore || softScores?.payoff_score || softScores?.monologue_score || 0}
          ...
          {/* Hook Multiplier Info */}
          {clip.hookMultiplier && clip.hookMultiplier !== 1.0 && (
            <div className="pt-2 border-t border-zinc-800/50">
              <div className="flex items-center gap-2">
                <span className="text-[10px] text-zinc-500">Hook Multiplier:</span>
                <span className={`text-xs font-medium ${
                  clip.hookMultiplier > 1 ? 'text-emerald-400' : 'text-amber-400'
                }`}>
                  {clip.hookMultiplier.toFixed(2)}x
                </span>
              </div>
            </div>
          )}
