...
ipcMain.handle('load-cached-transcript', async (_event, filePath: string) => {
  try {
    // Hash the file to find its cache directory
    ...
    const cacheDir = path.join(app.getPath('userData'), 'cache', fileHash);
    const transcriptPath = path.join(cacheDir, 'transcript.json');

    if (!fs.existsSync(transcriptPath)) {
      return { success: false, error: 'No cached transcript found' };
    }

    const transcript = JSON.parse(fs.readFileSync(transcriptPath, 'utf-8'));
    return { success: true, transcript };
  } catch (err) {
    console.error('[FileHandlers] Error loading cached transcript:', err);
