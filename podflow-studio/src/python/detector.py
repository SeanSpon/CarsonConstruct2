...
def send_result(
    ...
    if transcript_available is not None:
        result["transcriptAvailable"] = bool(transcript_available)
    if transcript_error:
        result["transcriptError"] = str(transcript_error)
    if transcript_source:
        result["transcriptSource"] = str(transcript_source)
...
def run_mvp_pipeline(video_path: str, settings: dict):
    ...
    def _transcribe_with_openai_whisper() -> dict:
        ...
        """
        if not openai_key:
            return {"segments": [], "words": [], "text": "", "error": "OpenAI API key missing"}
        ...
        try:
            send_progress(24, f"OpenAI transcript: {len(result.get('segments', []))} segments, {len(result.get('words', []))} words")
        except Exception:
            pass
        return result
    if not should_skip_stage(transcript_path, force):
        ...
        if existing_transcript and existing_transcript.get("segments"):
            ...
            try:
                ...
                send_progress(22, "Transcribing with word-level timestamps...")
                ...
                segments_list, _ = model.transcribe(
                    ...
                    language="en", 
                    vad_filter=True,
                    word_timestamps=True  # CRITICAL for karaoke-style captions
                )
                
                # Convert to standard format with word-level data
                segments = []
                words = []
                ...
                send_progress(24, f"Transcribed: {len(segments)} segments, {len(words)} words")
            except ImportError:
                send_progress(20, "faster-whisper not installed")
                if openai_key:
                    send_progress(20, "Falling back to OpenAI Whisper API...")
                    transcript_source = "openai-whisper"
                    transcript = _transcribe_with_openai_whisper()
                    transcript_error = transcript.get("error")
                else:
                    transcript_source = "none"
                    transcript_error = "faster-whisper not installed, and no OpenAI API key provided"
                    transcript = {"segments": [], "words": [], "text": ""}
            except Exception as e:
                send_progress(20, f"Transcription error: {e}")
                if openai_key:
                    send_progress(20, "Falling back to OpenAI Whisper API...")
                    transcript_source = "openai-whisper"
                    transcript = _transcribe_with_openai_whisper()
                    transcript_error = transcript.get("error") or f"Local transcription failed: {e}"
                else:
                    transcript_source = "faster-whisper"
                    transcript_error = f"Transcription error: {e}"
                    send_progress(20, "Continuing without transcript...")
                    transcript = {"segments": [], "words": [], "text": ""}
    ...
    transcript_available = bool(transcript and transcript.get("segments"))
